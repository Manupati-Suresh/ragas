...
class RAGASEvaluator:
    def __init__(self, rag_pipeline):
        ...
        # Set environment variable to use local models for RAGAS
        os.environ["RAGAS_USE_LOCAL_MODELS"] = "true"
    ...
    def create_evaluation_dataset(self, num_samples=50):
        ...
        for query in selected_queries:
            ...
            # Generate answer using the RAG pipeline (with fallback to simple formatting)
            try:
                answer = self.rag_pipeline.answer_query(query, method="hybrid", use_llm=True)
            except Exception as e:
                print(f"LLM generation failed for query '{query[:30]}...', using simple formatting")
                answer = self.rag_pipeline.answer_query(query, method="hybrid", use_llm=False)
    ...
    def configure_ragas_metrics(self):
        """
        Configure RAGAS metrics for evaluation using only open source models.
        """
        try:
            # Configure metrics to use local models
            metrics = [
            ...
            print("RAGAS metrics configured with local models:")
            ...
            print(f"Error configuring RAGAS metrics: {e}")
            print("Using simplified metrics configuration...")
            # Fallback to basic metrics
            return [context_precision, context_recall]
    
    def run_evaluation(self, dataset=None, metrics=None):
        """
        Run RAGAS evaluation on the dataset with enhanced error handling.
        """
        if dataset is None:
        ...
        try:
            ...
            print("This might be due to model loading issues or configuration problems.")
            print("Trying with simplified evaluation...")
            
            # Try with a smaller dataset and fewer metrics
            try:
                small_dataset = dataset.select(range(min(5, len(dataset))))
                simple_metrics = [context_precision, context_recall]
                ...
                print("Simplified RAGAS evaluation completed successfully!")
                return self.results
                
            except Exception as e2:
                print(f"Simplified evaluation also failed: {e2}")
                print("RAGAS evaluation requires proper model setup.")
                return None
    ...
    def analyze_results(self):
        ...
        scores = getattr(self.results, "scores", None)
        if isinstance(scores, dict):
            for metric_name, score in scores.items():
                if isinstance(score, (int, float)):
                    print(f"{metric_name}: {score:.4f}")
        else:
            print("No overall scores available in results (results may not be a RAGAS EvaluationResult).")

        # Convert to DataFrame for detailed analysis
        to_pandas = getattr(self.results, "to_pandas", None)
        if callable(to_pandas):
            ...
            print("No detailed DataFrame available in results.")
            return None
    
    def save_results(self, filename="ragas_evaluation_results.json", csv_filename="ragas_evaluation_detailed.csv"):
        """
        ...
        if self.results is None:
            ...
            return
        # Save overall scores as JSON
        scores = getattr(self.results, "scores", None)
        if isinstance(scores, dict):
            with open(filename, 'w') as f:
                json.dump(scores, f, indent=2)
            print(f"Overall scores saved to {filename}")
        else:
            print("No overall scores to save (results may not be a RAGAS EvaluationResult).")
        ...
        if callable(to_pandas):
            ...
            print("No detailed DataFrame to save.")
